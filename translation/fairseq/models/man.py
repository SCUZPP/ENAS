...
class MANEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, mix=False, no_man=False):
        ...
        self.no_man = no_man
        n_layernorm = 0
        self.encoder_seq = args.encoder_seq
        cnt = 0
        for i in range (len(args.encoder_seq)):
            #ff
            if args.encoder_seq[i] == 1:
                t = Linear(self.embed_dim, args.encoder_para[i] * self.embed_dim)
                self.add_module('sublayer_%d' % cnt, t)
                cnt += 1
                self.encoder_fc1.append(t)
                
                t = Linear(args.encoder_para[i] * self.embed_dim, self.embed_dim)
                self.add_module('sublayer_%d' % cnt, t)
                cnt += 1
            
                self.encoder_fc2.append(t)
            #sa   
            elif args.encoder_seq[i] == 2:
                t = MultiheadAttention(
                    self.embed_dim, args.encoder_para[i],
                    dropout=args.attention_dropout,
                )
                self.add_module('sublayer_%d' % cnt, t)
                cnt += 1
                self.encoder_self_attn.append(t)     
                
            #dsa   
            elif args.encoder_seq[i] == 3:
                t = MultiheadAttention(
                    self.embed_dim, args.encoder_para[i],
                    dropout=args.attention_dropout,
                    re_weight_m=1,
                ...
                self.add_module('sublayer_%d' % cnt, t)
                cnt += 1
                self.encoder_d_self_attn.append(t)
                
            n_layernorm += 1
        
        self.layer_norms = nn.ModuleList([LayerNorm(self.embed_dim) for i in range(n_layernorm)])
        """
        for i in self.encoder_fc1:
            i.cuda()
            
        for i in self.encoder_fc2:
            i.cuda()
            
        for i in self.encoder_self_attn:
            i.cuda()
            
        for i in self.encoder_d_self_attn:
            i.cuda()
        """
    def forward(self, x, encoder_padding_mask):
        ...
        for i in range (len(self.encoder_seq)):
            #ff
            if self.encoder_seq[i] == 1:
                residual = x
                x = self.maybe_layer_norm(i, x, before=True)
                x = F.relu(self.encoder_fc1[index1](x))
                x = F.dropout(x, p=self.relu_dropout, training=self.training)
                x = self.encoder_fc2[index1](x)
                x = F.dropout(x, p=self.dropout, training=self.training)
                x = residual + x
            ...
            #sa 
            elif self.encoder_seq[i] == 2:
                residual = x
                x = self.maybe_layer_norm(i, x, before=True)
                x, _ = self.encoder_self_attn[index2](query=x, key=x, value=x, key_padding_mask=encoder_padding_mask)
                x = F.dropout(x, p=self.dropout, training=self.training)
                x = residual + x
                x = self.maybe_layer_norm(i, x, after=True)
                index2 += 1
            ...
            #dsa  
            elif self.encoder_seq[i] == 3:
                residual = x
                x = self.maybe_layer_norm(i, x, before=True)
                x, _ = self.encoder_d_self_attn[index3](query=x, key=x, value=x, key_padding_mask=encoder_padding_mask)
                x = F.dropout(x, p=self.dropout, training=self.training)
                x = residual + x
                x = self.maybe_layer_norm(i, x, after=True)
                index3 += 1
...
class MANDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, no_encoder_attn=False, mix=False, no_man=False):
        ...
        for i in range(len(args.decoder_seq)):
            ...
            if args.decoder_seq[i] == 1:
                t = Linear(self.embed_dim, args.decoder_para[i] * self.embed_dim)
                self.add_module('sublayer_%d' % cnt, t)
                cnt += 1
                self.decoder_fc1.append(t)
                
                t = Linear(args.decoder_para[i] * self.embed_dim, self.embed_dim)
                self.add_module('sublayer_%d' % cnt, t)
                cnt += 1
            ...
            #sa
                ...
                t = MultiheadAttention(
                    self.embed_dim, args.decoder_para[i],
                    dropout=args.attention_dropout,
                )
                self.add_module('sublayer_%d' % cnt, t)
                cnt += 1
        ...
        #print('decoder_fc1', self.decoder_fc1)
        #print(self)
    ...
    def forward(self, x, encoder_out, encoder_padding_mask, incremental_state,
        ...
        for i in range(len(self.decoder_seq)):
            
            #ff
            if self.decoder_seq[i] == 1:
                residual = x
                x = self.maybe_layer_norm(self.normalize[i], x, before=True)
                x = F.relu(self.decoder_fc1[index1](x))
                x = F.dropout(x, p=self.relu_dropout, training=self.training)
                x = self.decoder_fc2[index1](x)
                x = F.dropout(x, p=self.dropout, training=self.training)
                x = residual + x
                x = self.maybe_layer_norm(self.normalize[i], x, after=True)              
                index1 += 1
                
            #sa
            elif self.decoder_seq[i] == 2:
                residual = x
                x = self.maybe_layer_norm(self.normalize[i], x, before=True)
                if prev_self_attn_state is not None:
                    #print('sa prev_self_attn_state')
                    if incremental_state is None:
                        incremental_state = {}
                    prev_key, prev_value = prev_self_attn_state
                    saved_state = {"prev_key": prev_key, "prev_value": prev_value}
                    self.decoder_self_attn[index2]._set_input_buffer(incremental_state, saved_state)

                x, _ = self.decoder_self_attn[index2](
                    query=x,
                    key=x,
                    value=x,
            ...
            #dsa
            elif self.decoder_seq[i] == 3:
                residual = x
                x = self.maybe_layer_norm(self.normalize[i], x, before=True)
                if prev_pre_self_attn_state is not None:
                    #print('dsa prev_self_attn_state')
                    if incremental_state is None:
                        incremental_state = {}
                    prev_key, prev_value = prev_pre_self_attn_state
                    saved_state = {"prev_key": prev_key, "prev_value": prev_value}
                    self.decoder_d_self_attn[index3]._set_input_buffer(incremental_state, saved_state)

                x, _ = self.decoder_d_self_attn[index3](
                    query=x,
                    key=x,
                    value=x,
                    key_padding_mask=self_attn_padding_mask,
            ...
            #ma
            elif self.decoder_seq[i] == 4:  
                attn = None
                if self.decoder_ma[index4] is not None:
                    residual = x
                    x = self.maybe_layer_norm(self.normalize[i], x, before=True)
                    if prev_attn_state is not None:
                        #print('ma prev_attn_state')
                        if incremental_state is None:
                            incremental_state = {}
                        prev_key, prev_value = prev_attn_state
                        saved_state = {"prev_key": prev_key, "prev_value": prev_value}
                        self.decoder_ma[index4]._set_input_buffer(incremental_state, saved_state)

                    x, attn = self.decoder_ma[index4](
                        query=x,
                        key=encoder_out,
                        value=encoder_out,
