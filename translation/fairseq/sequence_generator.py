...
class SequenceGenerator(object):
    ...
    def _generate(self, encoder_input, beam_size=None, maxlen=None, prefix_tokens=None):
        ...
        for model in self.models:
            ...
            #print('generate incremental_states len', len(incremental_states[model]))
        ...
        for step in range(maxlen + 1):  # one extra step for EOS marker
            ...
            if reorder_state is not None:
                ...
                for i, model in enumerate(self.models):
                    if isinstance(model.decoder, FairseqIncrementalDecoder):
                        #print('reorder_incremental_state')
                        #print('reorder_state', reorder_state)
                        model.decoder.reorder_incremental_state(incremental_states[model], reorder_state)
                    #print('prev reorder encoder', encoder_outs[i]['encoder_out'].shape)
                    encoder_outs[i] = model.encoder.reorder_encoder_out(encoder_outs[i], reorder_state)
                    #print('after reorder encoder', encoder_outs[i]['encoder_out'].shape)
            #print('!prev _decode incrmental_states len', len(incremental_states[model]))
            #for k in incremental_states:
            #    if 'prev_key' in incremental_states[model][k]:
            #        print('prev _decode ', k, incremental_states[model][k]['prev_key'].shape)
    ...
    def _decode(self, tokens, encoder_outs, incremental_states):
        """
        print('prev _decode incrmental_states len', len(incremental_states[self.models[0]]))
        for k in incremental_states[self.models[0]]:
            if 'prev_key' in incremental_states[self.models[0]][k]:
                print('_decode ', k, incremental_states[self.models[0]][k]['prev_key'].shape)
         """    
        #print('_decode')
    ...
    def _decode_one(self, tokens, model, encoder_out, incremental_states, log_probs):
        """
        print('prev _decode_one incrmental_states len', len(incremental_states[model]))
        for k in incremental_states[model]:
            if 'prev_key' in incremental_states[model][k]:
                #print('_decode_one', k, incremental_states[model][k]['prev_key'].shape)
        """
        with torch.no_grad():
            if incremental_states[model] is not None:
                #print('_decode_one')
                #print('encoder_out', encoder_out['encoder_out'].shape)
                #print('encoder_padding_mask', encoder_out['encoder_padding_mask'].shape)
